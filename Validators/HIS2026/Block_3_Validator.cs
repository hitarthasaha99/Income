...
namespace Income.Validators.HIS2026
{
    public class Block_3_Validator : AbstractValidator<Tbl_Block_3>
    {
        public Block_3_Validator()
        {
            ...
            RuleFor(x => x.item_12).NotNull().WithMessage("H0012(ii): Invalid Entry, Please check the entry").InclusiveBetween(1, 3).WithMessage("H0012(ii): Invalid Entry, Please check the entry").When(x => x.age >= 15);
