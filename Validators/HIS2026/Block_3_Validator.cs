...
namespace Income.Validators.HIS2026
{
    public class Block_3_Validator : AbstractValidator<Tbl_Block_3>
    {
        public Block_3_Validator()
        {
            ...
            RuleFor(x => x.item_6).Equal(2).When(x => x.item_3 == 2).WithMessage("H007(ii): Invalid Entry, Please check the entries recorded against cols. 3 & 6");
            RuleFor(x => x.item_6).Must((model, item6) => item6 != 1).When(x => x.item_3 == 3 || x.item_3 == 4).WithMessage("H007(iii): Invalid Entry, Please check the entries recorded against cols. 3 & 6");
            RuleFor(x => x.item_6).Must((model, item6) => item6 == 1).When(x => x.item_3 == 5).WithMessage("H007(iv): Invalid Entry, Please check the entries recorded against cols. 3 & 6");
